...
export default function FlowEditor() {
  ...
  const isFastFill = draft.mode === "fast_fill";
  ...
  return (
    <div className="px-6 md:px-10 py-8 max-w-[1400px] mx-auto">
      ...
      <PageHeader
        eyebrow={isFastFill ? `fast fill · v${draft.version || 1}` : `flow · v${draft.version || 1}`}
        title={draft.name || "Untitled flow"}
        description={
          isFastFill
            ? `Fast fill · ${draft.fast_fill?.domain || "no domain set"} · auto-submit ${draft.fast_fill?.auto_submit ? "on" : "off"}`
            : `${(draft.steps || []).length} steps · ${Math.round((draft.repair_confidence ?? 1) * 100)}% repair confidence`
        }
        actions={
          <>
            <Button
          ...
          {isFastFill ? (
            <div className="rounded-xl border border-border bg-card p-5">
              <div className="flex items-center gap-2 mb-2">
                ...
                <div className="text-sm font-medium">Fast Fill mode</div>
              </div>
              <p className="text-xs text-muted-foreground">
                Instead of a multi-step recorder, this flow injects the selected credential into
                the username & password fields and (optionally) clicks submit — in one action.
              </p>
            </div>
          ) : (
            <ProfilingSummary
